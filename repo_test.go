...
func TestAlpha(t *testing.T) {
	...
	query.Find("body h3 ~ ul").Each(func(_ int, s *goquery.Selection) {
		testList(t, s)
	})
}
...
func TestDuplicatedLinks(t *testing.T) {
	...
	query.Find("body h3 ~ ul a").Each(func(_ int, s *goquery.Selection) {
		href, ok := s.Attr("href")
...
func testList(t *testing.T, list *goquery.Selection) {
	list.Find("h3 ~ ul").Each(func(_ int, items *goquery.Selection) {
		testList(t, items)
